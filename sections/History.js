import Jumbotron from 'components/Jumbotron';
import ExternalLink from 'components/ExternalLink';

const History = () => (
  <section className="px-auto">
    <Jumbotron
      image="/bakers.jpg"
      alt="Joel and Nicole Baker"
    >
      Nicola and Joel owe a great debt of thankfulness to Eastern Europeans where they lived and ministered for
      over thirty years. Nicola’s family owes a debt to the Czechs. Her family lived in a house in Wimbledon,
      southwest London, England during World War 2. Nightly, the German air force bombed London and the residents
      slept in the Underground metro tunnels or concrete bunkers in their gardens. One night, Kath, her mother,
      despaired of sleeping in the cold bunker, and she just wanted to sleep in her own bed. That night, 6 one
      ton bombs landed on her street but didn’t explode! The army said they were sabotaged with sand instead of
      explosives at the Skoda munitions factory near Prague! Ten years later, Nicola was born.
    </Jumbotron>
    <Jumbotron
      reversed
      image="/kamenari.jpg"
      className="bg-white"
      alt="View of Bay of Kotor fron Kamenari village"
    >
      Joel had just received Christ as personal Savior just before starting college to study medicine near Chicago.
      At college, Joel met a fellow medical student, Lidija Jovanovic, from a Yugoslav family (with a Montenegrine name).
      Lidija explained the fullness of the Holy Spirit and took him to her Full Gospel Church in Chicago where Joel had a
      glorious supernatural baptism of the Holy Spirit (
      <ExternalLink href="https://www.kingjamesbibleonline.org/Acts-Chapter-8/#12">
        Acts 8:12-17
      </ExternalLink>
      ). In the former Yugoslavia, there is a lot of religious Christianity but unfortunately very little experience with
      the Holy Spirit! Joel feels a debt of thankfulness to help bless the people of the former Yugoslavia.
    </Jumbotron>
    <Jumbotron
      image="/joel.jpg"
      alt="Joel preaching"
    >
      Joel felt the “strength” of salvation (
      <ExternalLink href="https://www.kingjamesbibleonline.org/Romans-Chapter-5/#6">
        Rom. 5:6
      </ExternalLink>
      ), but was soon impressed by the Holy Spirit that there was more “power” to live for Christ and help others (
      <ExternalLink href="https://www.kingjamesbibleonline.org/Acts-Chapter-1/#8">
        Acts 1:8
      </ExternalLink>
      ). The 12 disciples were saved as they saw the risen Christ on that first Resurrection Sunday evening. They believed,
      and received the Holy Spirit “inside” them (
      <ExternalLink href="https://www.kingjamesbibleonline.org/John-Chapter-20/#22">
        Jn. 20:22
      </ExternalLink>). However, Jesus still commanded them not to leave Jerusalem, but to wait another 47 days for the
      outpouring of the Holy Spirit to come “upon” them (
      <ExternalLink href="https://www.kingjamesbibleonline.org/Acts-Chapter-1/#3s">
        Acts 1:3-8
      </ExternalLink>
      ). See his study notes on the “30 contrasts between the two main experiences with the Holy Spirit”. Come to Jesus
      and get everything He paid for you to have as soon as possible!
    </Jumbotron>
    <Jumbotron
      reversed
      image="/bible.jpg"
      className="bg-white"
      alt="Bible"
    >
      For this purpose, we want to bless you with many life-transforming revelations from the Word of God. We share
      with you freely 40 years of Bible Study notes, illustrations, articles and videos! Be blessed from our pastoral
      ministry over 6 churches, travelling ministry to 16 countries, 50 U.K. churches and teaching at 14 Bible Colleges!
      In His matchless love and grace, Joel and Nicola Baker
    </Jumbotron>
  </section>
);

export default History;
